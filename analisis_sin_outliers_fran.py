...
# ELIMINAR OUTLIERS - Conservar solo valores dentro de los límites por columna
df_sin_outliers = df.copy()

for i, col in enumerate(['Press_mm_hg', 'Windspeed', 'Visibility']):
    lower_limit = lower_whisker_series[i]
    upper_limit = upper_whisker_series[i]
    
    # Filtrar solo los valores dentro del rango para cada columna
    mask = (df_sin_outliers[col] >= lower_limit) & (df_sin_outliers[col] <= upper_limit)
    df_sin_outliers = df_sin_outliers[mask]

print(f"Filas originales: {len(df)}")
print(f"Filas después de eliminar outliers: {len(df_sin_outliers)}")

# Crear estadísticas sin outliers
df_stats_sin_outliers = df_sin_outliers[columns_describe]
...
print('='*60)
print(f'{MAGENTA}WINSORIZATION APLICADA{RESET}')
print('• Outliers reemplazados por los límites IQR')
print('• Datos listos para clustering')
print('='*60)

# --- Grilla de gráficos CON DATOS WINSORIZADOS ---
df_meteorologicos_pt2 = df_sin_outliers[columns].copy()
...
# 1. Line chart of Press_mm_hg (CON WINSORIZATION)
ax1.plot(df_meteorologicos_pt2['date'], df_meteorologicos_pt2['Press_mm_hg'], label='Press_mm_hg (mmHg)', color='purple', linewidth=1, alpha=0.8)
ax1.set_title("Presión atmosférica a lo largo del tiempo (Datos Winsorizados)")
ax1.set_xlabel("Fecha")
ax1.set_ylabel("Presión (mmHg)")
ax1.legend()
ax1.grid(True)

# 2. Line charts of Windspeed and Visibility (CON WINSORIZATION)
ax2_twin = ax2.twinx()  # Create second y-axis
ax2.plot(df_meteorologicos_pt2['date'], df_meteorologicos_pt2['Windspeed'], label='Windspeed (m/s)', color='orange', linewidth=1, alpha=0.8)
ax2_twin.plot(df_meteorologicos_pt2['date'], df_meteorologicos_pt2['Visibility'], label='Visibility (km)', color='cyan', linewidth=1, alpha=0.8)
ax2.set_title("Velocidad del viento y Visibilidad a lo largo del tiempo (Datos Winsorizados)")
...
# 3. Histogram for Press_mm_hg (CON WINSORIZATION)
mean_press = df_stats_sin_outliers['Press_mm_hg'].mean()
median_press = df_stats_sin_outliers['Press_mm_hg'].median()
std_press = df_stats_sin_outliers['Press_mm_hg'].std()
n, bins, patches = ax3.hist(df_stats_sin_outliers['Press_mm_hg'], bins=30, density=False, alpha=0.7, color='purple', edgecolor='black')

# Agregar líneas verticales para media y mediana
ax3.axvline(mean_press, color='blue', linestyle='--', linewidth=2, label=f'Media: {mean_press:.2f} mmHg')
ax3.axvline(median_press, color='green', linestyle='--', linewidth=2, label=f'Mediana: {median_press:.2f} mmHg')
ax3.axvline(lower_whisker_series[0], color='orange', linestyle=':', linewidth=2, label=f'Límite inf: {lower_whisker_series[0]:.2f} mmHg')
ax3.axvline(upper_whisker_series[0], color='red', linestyle=':', linewidth=2, label=f'Límite sup: {upper_whisker_series[0]:.2f} mmHg')
ax3.legend()

ax3.set_title(f'Distribución de Press_mm_hg (Winsorized)\n(μ={mean_press:.2f}, σ={std_press:.2f})')
ax3.set_xlabel('Presión atmosférica (mmHg)')
ax3.set_ylabel('Frecuencia')
ax3.grid(True, alpha=0.3)

# 4. Histogram for Windspeed (CON WINSORIZATION)
mean_wind = df_stats_sin_outliers['Windspeed'].mean()
median_wind = df_stats_sin_outliers['Windspeed'].median()
std_wind = df_stats_sin_outliers['Windspeed'].std()
n, bins, patches = ax4.hist(df_stats_sin_outliers['Windspeed'], bins=30, density=False, alpha=0.7, color='orange', edgecolor='black')

# Agregar líneas verticales para media y mediana
ax4.axvline(mean_wind, color='blue', linestyle='--', linewidth=2, label=f'Media: {mean_wind:.2f} m/s')
ax4.axvline(median_wind, color='red', linestyle='--', linewidth=2, label=f'Mediana: {median_wind:.2f} m/s')
ax4.axvline(lower_whisker_series[1], color='green', linestyle=':', linewidth=2, label=f'Límite inf: {lower_whisker_series[1]:.2f} m/s')
ax4.axvline(upper_whisker_series[1], color='purple', linestyle=':', linewidth=2, label=f'Límite sup: {upper_whisker_series[1]:.2f} m/s')
ax4.legend()

ax4.set_title(f'Distribución de Windspeed (Winsorized)\n(μ={mean_wind:.2f}, σ={std_wind:.2f})')
ax4.set_xlabel('Velocidad del viento (m/s)')
ax4.set_ylabel('Frecuencia')
ax4.grid(True, alpha=0.3)

# 5. Histogram for Visibility (CON WINSORIZATION)
mean_vis = df_stats_sin_outliers['Visibility'].mean()
median_vis = df_stats_sin_outliers['Visibility'].median()
std_vis = df_stats_sin_outliers['Visibility'].std()
n, bins, patches = ax5.hist(df_stats_sin_outliers['Visibility'], bins=30, density=False, alpha=0.7, color='cyan', edgecolor='black')

# Agregar líneas verticales para media y mediana
ax5.axvline(mean_vis, color='red', linestyle='--', linewidth=2, label=f'Media: {mean_vis:.2f} km')
ax5.axvline(median_vis, color='green', linestyle='--', linewidth=2, label=f'Mediana: {median_vis:.2f} km')
ax5.axvline(lower_whisker_series[2], color='orange', linestyle=':', linewidth=2, label=f'Límite inf: {lower_whisker_series[2]:.2f} km')
ax5.axvline(upper_whisker_series[2], color='purple', linestyle=':', linewidth=2, label=f'Límite sup: {upper_whisker_series[2]:.2f} km')
ax5.legend()

ax5.set_title(f'Distribución de Visibility (Winsorized)\n(μ={mean_vis:.2f}, σ={std_vis:.2f})')
ax5.set_xlabel('Visibilidad (km)')
ax5.set_ylabel('Frecuencia')
ax5.grid(True, alpha=0.3)

# 6. Boxplot for Press_mm_hg (CON WINSORIZATION)
sns.boxplot(data=df_stats_sin_outliers['Press_mm_hg'], ax=ax6, color='purple', orient="v")
ax6.set_title("Diagrama de cajas para Press_mm_hg (Winsorized)")
ax6.set_ylabel("Presión atmosférica (mmHg)")

# 7. Boxplot for Windspeed (CON WINSORIZATION)
sns.boxplot(data=df_stats_sin_outliers['Windspeed'], ax=ax7, color='orange', orient="v")
ax7.set_title("Diagrama de cajas para Windspeed (Winsorized)")
ax7.set_ylabel("Velocidad del viento (m/s)")

# 8. Boxplot for Visibility (CON WINSORIZATION)
sns.boxplot(data=df_stats_sin_outliers['Visibility'], ax=ax8, color='cyan', orient="v")
ax8.set_title("Diagrama de cajas para Visibility (Winsorized)")
ax8.set_ylabel("Visibilidad (km)")

# 9. Hourly pattern for Press_mm_hg (CON WINSORIZATION)
...
ax9.set_title('Press_mm_hg por Hora del Día (± desviación estándar) - Winsorized')
...
# 10. Hourly pattern for Windspeed (CON WINSORIZATION)
...
ax10.set_title('Windspeed por Hora del Día (± desviación estándar) - Winsorized')
...
# 11. Hourly pattern for Visibility (CON WINSORIZATION)
...
ax11.set_title('Visibility por Hora del Día (± desviación estándar) - Winsorized')
