...
df_meteorologicos_pt1 = df_sin_outliers[columns].copy()
...
# 1. Line chart of T_out and Tdewpoint (CON WINSORIZATION)
ax1.plot(df_meteorologicos_pt1['date'], df_meteorologicos_pt1['T_out'], label='T_out (°C)', color='red', linewidth=1, alpha=0.8)
ax1.plot(df_meteorologicos_pt1['date'], df_meteorologicos_pt1['Tdewpoint'], label='Tdewpoint (°C)', color='green', linewidth=1, alpha=0.8)
ax1.set_title("Temperaturas a lo largo del tiempo (Datos Winsorizados)")
ax1.set_xlabel("Fecha")
ax1.set_ylabel("Grados Celsius °C")
ax1.legend()
ax1.grid(True)

# 2. Line chart of RH_out (CON WINSORIZATION)
ax2.plot(df_meteorologicos_pt1['date'], 
    ...
    linewidth=1,
    alpha=0.8)
ax2.set_title("Humedad exterior a lo largo del tiempo (Datos Winsorizados)")
ax2.set_xlabel("Fecha")
ax2.set_ylabel("Humedad (%)")
ax2.legend()
ax2.grid(True)

# 3. Histogram for T_out (CON WINSORIZATION)
mean_t_out = df_stats_sin_outliers['T_out'].mean()
median_t_out = df_stats_sin_outliers['T_out'].median()
std_t_out = df_stats_sin_outliers['T_out'].std()
n, bins, patches = ax3.hist(df_stats_sin_outliers['T_out'], bins=30, density=False, alpha=0.7, color='red', edgecolor='black')

# Agregar líneas verticales para media y mediana
ax3.axvline(mean_t_out, color='blue', linestyle='--', linewidth=2, label=f'Media: {mean_t_out:.2f}°C')
ax3.axvline(median_t_out, color='green', linestyle='--', linewidth=2, label=f'Mediana: {median_t_out:.2f}°C')
ax3.axvline(lower_whisker_series[0], color='orange', linestyle=':', linewidth=2, label=f'Límite inf: {lower_whisker_series[0]:.2f}°C')
ax3.axvline(upper_whisker_series[0], color='purple', linestyle=':', linewidth=2, label=f'Límite sup: {upper_whisker_series[0]:.2f}°C')
ax3.legend()

ax3.set_title(f'Distribución de T_out (Winsorized)\n(μ={mean_t_out:.2f}, σ={std_t_out:.2f})')
ax3.set_xlabel('T_out (°C)')
ax3.set_ylabel('Frecuencia')
ax3.grid(True, alpha=0.3)

# 4. Histogram for Tdewpoint (CON WINSORIZATION)
mean_tdew = df_stats_sin_outliers['Tdewpoint'].mean()
median_tdew = df_stats_sin_outliers['Tdewpoint'].median()
std_tdew = df_stats_sin_outliers['Tdewpoint'].std()
n, bins, patches = ax4.hist(df_stats_sin_outliers['Tdewpoint'], bins=30, density=False, alpha=0.7, color='green', edgecolor='black')

# Agregar líneas verticales para media y mediana
ax4.axvline(mean_tdew, color='blue', linestyle='--', linewidth=2, label=f'Media: {mean_tdew:.2f}°C')
ax4.axvline(median_tdew, color='red', linestyle='--', linewidth=2, label=f'Mediana: {median_tdew:.2f}°C')
ax4.axvline(lower_whisker_series[1], color='orange', linestyle=':', linewidth=2, label=f'Límite inf: {lower_whisker_series[1]:.2f}°C')
ax4.axvline(upper_whisker_series[1], color='purple', linestyle=':', linewidth=2, label=f'Límite sup: {upper_whisker_series[1]:.2f}°C')
ax4.legend()

ax4.set_title(f'Distribución de Tdewpoint (Winsorized)\n(μ={mean_tdew:.2f}, σ={std_tdew:.2f})')
ax4.set_xlabel('Tdewpoint (°C)')
ax4.set_ylabel('Frecuencia')
ax4.grid(True, alpha=0.3)

# 5. Histogram for RH_out (CON WINSORIZATION)
mean_rh = df_stats_sin_outliers['RH_out'].mean()
median_rh = df_stats_sin_outliers['RH_out'].median()
std_rh = df_stats_sin_outliers['RH_out'].std()
n, bins, patches = ax5.hist(df_stats_sin_outliers['RH_out'], bins=30, density=False, alpha=0.7, color='blue', edgecolor='black')

# Agregar líneas verticales para media y mediana
ax5.axvline(mean_rh, color='red', linestyle='--', linewidth=2, label=f'Media: {mean_rh:.2f}%')
ax5.axvline(median_rh, color='green', linestyle='--', linewidth=2, label=f'Mediana: {median_rh:.2f}%')
ax5.axvline(lower_whisker_series[2], color='orange', linestyle=':', linewidth=2, label=f'Límite inf: {lower_whisker_series[2]:.2f}%')
ax5.axvline(upper_whisker_series[2], color='purple', linestyle=':', linewidth=2, label=f'Límite sup: {upper_whisker_series[2]:.2f}%')
ax5.legend()

ax5.set_title(f'Distribución de RH_out (Winsorized)\n(μ={mean_rh:.2f}, σ={std_rh:.2f})')
ax5.set_xlabel('RH_out (%)')
ax5.set_ylabel('Frecuencia')
ax5.grid(True, alpha=0.3)

# 6. Combined boxplot for T_out and Tdewpoint (CON WINSORIZATION)
sns.boxplot(data=df_stats_sin_outliers[['T_out','Tdewpoint']], ax=ax6, 
            palette=['red', 'green'], orient="h",)
ax6.set_title("Diagrama de cajas para T_out y Tdewpoint (Winsorized)")
ax6.set_xlabel("Valores (°C)")
ax6.set_ylabel("Variables")

# 7. Boxplot for RH_out (CON WINSORIZATION)
sns.boxplot(data=df_stats_sin_outliers['RH_out'], ax=ax7, color='blue', orient="h")
ax7.set_title("Diagrama de cajas para RH_out (Winsorized)")
ax7.set_ylabel("RH_out (%)")


# 8. Hourly pattern for T_out (CON WINSORIZATION)
...
ax8.set_title('T_out por Hora del Día (± desviación estándar) - Winsorized')
...
# 9. Hourly pattern for Tdewpoint (CON WINSORIZATION)
...
ax9.set_title('Tdewpoint por Hora del Día (± desviación estándar) - Winsorized')
...
# 10. Hourly pattern for RH_out (CON WINSORIZATION)
...
ax10.set_title('RH_out por Hora del Día (± desviación estándar) - Winsorized')
